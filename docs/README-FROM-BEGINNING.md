...
> **Note:** `kubectl` and `minikube` binaries are **not included in this repo**
> (too large for GitHub). Install them fresh using the commands above.
> The `k8s/*.yaml` manifest files ARE included in the repo — that's all you need from the repo side.
